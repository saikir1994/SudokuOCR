...
int main()
{
	//IplImage* pImg = cvLoadImage("sudoku.jpg");
	//Mat  sudoku(pImg);
	Mat sudoku = imread("sudoku.jpg", 0);
	//Mat original = sudoku,clone();
	/*imwrite("temp1.jpg", sudoku);

	if (sudoku.empty())
		cout << "empty"<<endl;
	Mat pano = imread("temp1.jpg", 0);
	sudoku = pano;*/
	imshow("result", sudoku);
	waitKey(0);
	if (sudoku.empty())
		cout << "empty"<<endl;
	//imshow("input", pano);
	...
	waitKey(0);
	cout << "undistorted image obtained"<<endl;
	...
	//tess.SetImage((uchar*)undistortedThreshed.data, undistortedThreshed.cols, undistortedThreshed.rows, 1, undistortedThreshed.cols);
	...
	for (int j = 0; j < 9; j++)
	{
		for (int i = 0; i < 9; i++)
		{
			for (int y = 0; y < dist && j*dist + y < undistortedThreshed.cols; y++)
			{
				...
				{
					if (y<(dist * 15) / 100 || y >(dist * 85) / 100)
					{
						ptr[x] = 0;
					}
					else if (x<(dist * 20) / 100 || x >(dist * 80) / 100)
					{
						ptr[x] = 0;
					}
					else
